...
export function getAdminApp() {
  ...
  if (!envVar) {
    firebaseConfig = { error: "FIREBASE_SERVICE_ACCOUNT environment variable is not set." };
  ...
  try {
    ...
    console.log(`[Firebase Admin] Attempting initialization. Apps length: ${admin.apps.length}`);
    ...
    if (admin.apps.length === 0) {
      ...
      console.log("[Firebase Admin] Reusing existing admin app instance.");
    }
    return adminApp;
  } catch (e) {
