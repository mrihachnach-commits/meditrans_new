...
const getDbId = () => firebaseConfig.firestoreDatabaseId || 'ai-studio-274cca7a-128b-4f84-92a2-6b79856d515b';

export const firestoreRest = {
  getDoc: async (collection, docId, idToken) => {
    const dbId = getDbId();
    const url = `https://firestore.googleapis.com/v1/projects/${firebaseConfig.projectId}/databases/${dbId}/documents/${collection}/${docId}`;
  ...
  listDocs: async (collection, idToken) => {
    const dbId = getDbId();
    const url = `https://firestore.googleapis.com/v1/projects/${firebaseConfig.projectId}/databases/${dbId}/documents/${collection}`;
    const headers = { 'Content-Type': 'application/json' };
    if (idToken) headers['Authorization'] = `Bearer ${idToken}`;
